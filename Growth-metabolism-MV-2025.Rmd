...
mass_mod_trtint <- lmer(log_mass ~ Month*as.factor(Temp_class) + Sex + Density + (0 + Temp_class|ID_fish), 
                 data=ds1, REML=T)
...
#SMR plotted over time, note that SMR is not mass-standardized and the mass range has changed over time as well.
ggplot(ds1, aes(x = factor(Month), y = SMR, color = factor(Temp_class))) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.6) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8), size = 2, alpha = 0.8) +
  labs(x = "Month", y = expression(SMR~(mg~O[2]~h^-1)), color = "Temperature") +
  theme_bw() +
  scale_color_manual(values = c("#78B7C5", "#F21A00")) +
  scale_y_log10()
...
SMR18_mod<-lmer(log10(SMR) ~ log_mass + (1 + Month_int|ID_fish), # random slopes by month, random intercepts by ID_fish
                 data=ds18, REML = T)
...
#model random slopes by mass
SMR_mod_mass <- lmer(log10(SMR) ~ log_mass*as.factor(Temp_class) + Sex + Density + (1 + log_mass|ID_fish),  
                 data=ds1, REML=T)
plot(SMR_mod_mass)                                             # generate model output
summary(SMR_mod_mass)                                          # plot model residuals
ranova(SMR_mod_mass)                                           # anova-like test for significance of model components
...
#RMR over time, note that month is found below to be insignificant as a random effect, and RMR plotted here is not mass-standardized.
ggplot(ds1, aes(x = factor(Month), y = RMR, color = factor(Temp_class))) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.6) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8), size = 2, alpha = 0.8) +
  labs(x = "Month", y = expression(RMR~(mg~O[2]~h^-1)), color = "Temperature") +
  theme_bw() +
  scale_color_manual(values = c("#78B7C5", "#F21A00")) +
  scale_y_log10()
...
#model random slopes by mass
RMR_mod_mass <- lmer(log10(RMR) ~ log_mass*as.factor(Temp_class) + Sex + Density + (1 + log_mass|ID_fish),  
                 data=ds1, REML=T)
plot(RMR_mod_mass)                                             # generate model output
summary(RMR_mod_mass)                                          # plot model residuals
ranova(RMR_mod_mass)                                           # anova-like test for significance of model components
...
#MMR over time, note that month is found below to be insignificant as a random effect, and MMR plotted here is not mass-standardized.
ggplot(ds1, aes(x = factor(Month), y = MMR, color = factor(Temp_class))) +
  geom_boxplot(position = position_dodge(width = 0.8), width = 0.6) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.8), size = 2, alpha = 0.8) +
  labs(x = "Month", y = expression(MMR~(mg~O[2]~h^-1)), color = "Temperature") +
  theme_bw() +
  scale_color_manual(values = c("#78B7C5", "#F21A00")) +
  scale_y_log10()
...
#model random slopes by mass
MMR_mod_mass <- lmer(log10(MMR) ~ log_mass*as.factor(Temp_class) + Sex + Density + (1 + log_mass|ID_fish),  
                 data=ds1, REML=T)
plot(MMR_mod_mass)                                             # generate model output
summary(MMR_mod_mass)                                          # plot model residuals
ranova(MMR_mod_mass)                                           # anova-like test for significance of model components
...
fig2.1 <- ggplot() +
  ...
  theme_classic() +
  ## --- add isometric scaling lines ---
  geom_abline(intercept = -1, slope = 1, linetype = "dashed", color = "grey50") +
  geom_abline(intercept = -0.5, slope = 1, linetype = "dashed", color = "grey50") +
...
fat_mod_sexlinked <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + Sex + Sex:Temp_class + Sex:log_finalmass_centre + Sex:Temp_class:log_finalmass_centre + Density, data=ds_fat)
...
anova(fat_mod2)
anova(fat_mod3)
anova(fat_mod_sexlinked)

```

## additional fat mass models to test alternative predictions:
### Prediction 1: Individual fat mass is the product of cumulative metabolic performance over time, not just a single point.

```{r}

# first step: add six new summary variables to ds_fat

ds_fat2 <- ds1 %>%
  group_by(ID_fish, Temp_class, Sex, Fat_mass_g, log_fatmass, log_finalmass_centre, finalmass_centre, Final_Mass_g) %>%
  summarise(
    sum_SMR = sum(SMR, na.rm = TRUE),
    mean_SMR = mean(SMR, na.rm = TRUE),
    sum_RMR = sum(RMR, na.rm = TRUE),
    mean_RMR = mean(RMR, na.rm = TRUE),
    sum_MMR = sum(MMR, na.rm = TRUE),
    mean_MMR = mean(MMR, na.rm = TRUE)
  ) %>% drop_na()

density <- ds1[which(ds1$Month == "September"),] %>% dplyr::select(ID_fish, Density)

ds_fat2 <- left_join(ds_fat2, density, by = c("ID_fish"))

ds_fat2 <- ds_fat2[which(ds_fat2$ID_fish != 99),] #Fish 99 is a clear outlier in the diagnostic plots. I have tested these models with and without it, and the results WRT the interaction terms between temperature:metabolism are unchanged.

# now we build the new models.
    # sum of metabolic trait models:

fat_mod_sumSMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + sum_SMR:Temp_class + Sex + Density, data=ds_fat2)

fat_mod_sumRMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + sum_RMR:Temp_class + Sex + Density, data=ds_fat2)

fat_mod_sumMMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + sum_MMR:Temp_class + Sex + Density, data=ds_fat2)

    # mean of metabolic trait models:

fat_mod_meanSMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + mean_SMR:Temp_class + Sex + Density, data=ds_fat2)

fat_mod_meanRMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + mean_RMR:Temp_class + Sex + Density, data=ds_fat2)

fat_mod_meanMMR <- lm(log_fatmass ~ Temp_class + log_finalmass_centre:Temp_class + mean_MMR:Temp_class + Sex + Density, data=ds_fat2)

# finally, we test those models using the summary function.

plot(fat_mod_sumSMR)
plot(fat_mod_sumRMR)
plot(fat_mod_sumMMR)

summary(fat_mod_sumSMR)
summary(fat_mod_sumRMR)
summary(fat_mod_sumMMR)

anova(fat_mod_sumSMR)
anova(fat_mod_sumRMR)
anova(fat_mod_sumMMR)

plot(fat_mod_meanSMR)
plot(fat_mod_meanRMR)
plot(fat_mod_meanMMR)

summary(fat_mod_meanSMR)
summary(fat_mod_meanRMR)
summary(fat_mod_meanMMR)

anova(fat_mod_meanSMR)
anova(fat_mod_meanRMR)
anova(fat_mod_meanMMR)

#none of these models indicate that any cumulative metabolic trait at any temperature is significantly correlated with fat mass accumulation. Our results do not support Prediction 1.
```

### Prediction 2: Individual mass-specific metabolic performance will decrease with increasing fat mass, as fat tissue is not considered metabolically active.
```{r}

# first, we build three new models:

SMR_fatmod <- lm(SMR ~ Temp_class + log_finalmass_centre:Temp_class + log_fatmass:Temp_class + Sex + Density, data=ds_fat)
RMR_fatmod <- lm(RMR ~ Temp_class + log_finalmass_centre:Temp_class + log_fatmass:Temp_class + Sex + Density, data=ds_fat)
MMR_fatmod <- lm(MMR ~ Temp_class + log_finalmass_centre:Temp_class + log_fatmass:Temp_class + Sex + Density, data=ds_fat)
 
# then we test those models using the summary function.
...
# here again, none of our models find any significant correlation between the interaction between temperature and fat mass with any metabolic trait. Our results do not support Prediction 2.
